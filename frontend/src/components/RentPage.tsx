...
export default function RentPage() {
    ...
    const handleEdit = async(id: number) => {
        navigate.push(`/admin/rent/edit/${id}`)
    }

    const handleDelete = async(id: number) => {
        try {
            const token = await axios.get('/service/token');
            const response = await axios.delete(`http://localhost:8000/api/a1/rent/${id}?token=${token.data.token}`)
                        ...
                        <table className="border-collapse font-[500] mx-auto">
                            <thead>
                                <tr>
                                    ...
                                    <td className="text-[18px] border-b-3 px-6 pb-3 border-black">Down Payment</td>
                            ...
                            <tbody>
                                {item.map((items, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="text-[16px] py-6 px-6 pb-3 border-black">Rp. {items.down_payment}</td>
