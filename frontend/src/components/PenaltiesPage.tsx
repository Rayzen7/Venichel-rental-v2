...
export default function PenaltiesPage() {
    ...
    useEffect(() => {
        const fetchItem = async() => {
            ...
            const response = await axios.get(`http://localhost:8000/api/a1/penalties?token=${token.data.token}`);
            setItem(response.data.penalties);
        }
    ...
    const handleEdit = async(id: number) => {
        navigate.push(`/admin/penalties/edit/${id}`)
    }

    const handleDelete = async(id: number) => {
        try {
            const token = await axios.get('/service/token');
            const response = await axios.delete(`http://localhost:8000/api/a1/penalties/${id}?token=${token.data.token}`)
                        ...
                        <table className="border-collapse font-[500] mx-auto">
                            <thead>
                                <tr>
                                    ...
                                    <td className="text-[18px] border-b-3 px-10 pb-3 border-black">Penalties Name</td>
                                    <td className="text-[18px] border-b-3 px-10 pb-3 border-black">Description</td>
                                    <td className="text-[18px] border-b-3 px-10 pb-3 border-black">No Car</td>
                                    <td className="text-[18px] border-b-3 px-10 pb-3 border-black">Penalties Total</td>                                    
                                    <td className="text-[18px] border-b-3 px-10 pb-3 border-black">Action</td>
                                </tr>
                            </thead>
                            <tbody>
                                {item.map((items, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="text-[16px] py-6 px-10 pb-3 border-black">{items.penalties_name}</td>
                                        <td className="text-[16px] py-6 px-10 pb-3 border-black">{items.description}</td>
                                        <td className="text-[16px] py-6 px-10 pb-3 border-black">{items.no_car}</td>
                                        <td className="text-[16px] py-6 px-10 pb-3 border-black">Rp. {items.penalties_total}</td>
                                        <td className="text-[16px] py-6 px-10 pb-3 flex justify-center items-center gap-2 border-black">
                                            <MagicButton
