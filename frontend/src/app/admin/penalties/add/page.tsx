...
export default function AddPenalties() {
    interface User {
        id: number;
        tenant: number;
        no_car: number;
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    try {
      const token = await axios.get('/service/token');
      const response = await axios.post(`http://localhost:8000/api/a1/penalties?token=${token.data.token}`, {
        no_car,
        penalties_name,
        penalties_total,
        description
        ...
        <div className="text-start w-full">
            <Link href='/admin/home'>
                <div className="flex justify-start group items-center gap-4">
                  ...
                  <h1 className='font-[600] text-[28px] text-primary'>Add Penalties</h1>    
                </div>
            </Link>
        </div>
        <div className="flex gap-24">
            ...
            <div className="">
                ...
                <div className="mt-14 gap-4 flex justify-center flex-col items-center">
                  <button onClick={handleSubmit} className='bg-primary text-white font-[500] text-[16px] w-[350px] cursor-pointer py-3 duration-200 rounded-md hover:bg-transparent hover:text-primary border-3 border-primary'>Add Penalties</button>
                </div>
            </div>
        </div>
      </div>
