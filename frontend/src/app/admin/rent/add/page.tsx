...
export default function AddRent() {
  ...
  useEffect(() => {
    ...
    fetchItem();
  }, [down_payment, discount]);
  
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault();
    try {
      const token = await axios.get('/service/token');
      const response = await axios.post(`http://localhost:8000/api/a1/rent?token=${token.data.token}`, {
        tenant,
        no_car,
        date_borrow,
        date_return,
        ...
        <div className="flex gap-24">
            ...
            <div className="">
                ...
                <div className="mt-14 gap-4 flex justify-center flex-col items-center">
                  <button onClick={handleSubmit} className='bg-primary text-white font-[500] text-[16px] w-[350px] cursor-pointer py-3 duration-200 rounded-md hover:bg-transparent hover:text-primary border-3 border-primary'>Add Rent</button>
                </div>
            </div>
        </div>
      </div>
