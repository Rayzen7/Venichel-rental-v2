...
export default function AddReturn() {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    try {
      const token = await axios.get('/service/token');
      const response = await axios.post(`http://localhost:8000/api/a1/return?token=${token.data.token}`, {
        tenant,
        no_car,
        date_borrow,
        date_return,
        ...
        <div className="text-start w-full">
            <Link href='/admin/home'>
                <div className="flex justify-start group items-center gap-4">
                  ...
                  <h1 className='font-[600] text-[28px] text-primary'>Add Return</h1>    
                </div>
            </Link>
        </div>
        <div className="flex gap-24">
                ...
                <div className="flex flex-col gap-6 justify-center mt-10">
                    <div className="flex flex-col gap-2">
                        ...
                        >
                            ...
                            {rent.length > 0 ? (
                                rent.map((rents) => (
                                    <option value={rents.id} key={rents.id}>{rents.tenant} | {rents.no_car} | Down Payment Rp. {rents.down_payment}</option>
                                ))
                            ) : (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <select name="" className='border-3 pointer-events-none appearance-none  p-3 rounded-md'
                      value={tenant} 
                      onChange={(e) => {
                        const val = e.target.value
                        setTenant(val);
            ...
            <div className="">
                ...
                <div className="mt-14 gap-4 flex justify-center flex-col items-center">
                  <button onClick={handleSubmit} className='bg-primary text-white font-[500] text-[16px] w-[350px] cursor-pointer py-3 duration-200 rounded-md hover:bg-transparent hover:text-primary border-3 border-primary'>Add Return</button>
                </div>
            </div>
        </div>
      </div>
